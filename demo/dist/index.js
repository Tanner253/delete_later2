...
__export(index_exports, {
  ...
  PortalTokenManager: () => PortalTokenManager,
  ...
  createPortalTokenManager: () => createPortalTokenManager,
  ...
  formatPortalAmount: () => formatPortalAmount,
...
var MemoryStorage = class {
  ...
  async savePaymentLink(paymentLink) {
    this.links.set(paymentLink.id, { ...paymentLink });
  }
  async updatePaymentLink(paymentLink) {
    if (!this.links.has(paymentLink.id)) {
      throw new Error(`Payment link ${paymentLink.id} not found`);
    }
    this.links.set(paymentLink.id, { ...paymentLink, updatedAt: /* @__PURE__ */ new Date() });
  }
  ...
  async getConfirmedPayment(paymentLinkId) {
  ...
  async getSubscriptionByAddress(paymentLinkId, subscriberAddress) {
    const addressKey = `${paymentLinkId}:${subscriberAddress}`;
    return this.subscriptionsByAddress.get(addressKey) ?? null;
  }
  async getSubscriptionsByPaymentLink(paymentLinkId) {
...
var WebhookManager = class {
   ...
   * Send payment event
   */
  async sendPaymentEvent(event, payment, paymentLink) {
   ...
   * Send link event
   */
  async sendLinkEvent(event, paymentLink) {
    if (!this.isEventEnabled(event)) {
      return null;
    }
    const payload = {
   ...
   */
  async sendSubscriptionEvent(event, subscription, paymentLink) {
    if (!this.isEventEnabled(event)) {
      return null;
    }
    const payload = {
...
// lib/protocol.ts
function build403Response(reasonCode, options = {}) {
  return {
...
var SubscriptionManager = class {
   ...
   */
  async createSubscription(paymentLink, input) {
   ...
   */
  async processPayment(subscription, payment, paymentLink) {
    if (!paymentLink.subscription) {
      throw new Error("PaymentLink is not configured for subscriptions");
    }
    const config = paymentLink.subscription;
   ...
   */
  async checkAccess(subscription, paymentLink) {
...
var PortalServer = class {
   ...
   */
  async handlePaymentLinkRequest(id) {
    ...
    if (link.status !== "active") {
      return {
        type: "forbidden",
        body: build403Response("LINK_DISABLED" /* LINK_DISABLED */, { paymentLinkId: link.id })
      };
    }
    if (isExpired(link.expiresAt)) {
      return {
        type: "forbidden",
        body: build403Response("LINK_EXPIRED" /* LINK_EXPIRED */, { paymentLinkId: link.id })
      };
    }
    if (isLimitReached(link.usedCount, link.maxUses)) {
      return {
        type: "forbidden",
        body: build403Response("LINK_USAGE_LIMIT_REACHED" /* LINK_USAGE_LIMIT_REACHED */, { paymentLinkId: link.id })
      };
    }
   ...
   */
  async confirmPayment(paymentLinkId, txHash) {
   ...
   */
  async createSubscription(paymentLinkId, subscriberAddress, metadata) {
    const link = await this.storage.getPaymentLink(paymentLinkId);
  ...
  // ========================================
  async handlePaymentLink(req, res) {
    try {
  ...
  send403(res, code, paymentLinkId, details) {
    const body = {
      protocol: "403-payportal-v1",
      paymentLinkId,
      reasonCode: code,
...
var MemoryStorage2 = class {
  ...
  payments = /* @__PURE__ */ new Map();
  paymentsByTxHash = /* @__PURE__ */ new Map();
  paymentsByPaymentLinkId = /* @__PURE__ */ new Map();
  subscriptions = /* @__PURE__ */ new Map();
  subscriptionsByAddress = /* @__PURE__ */ new Map();
  subscriptionsByPaymentLink = /* @__PURE__ */ new Map();
  async getPaymentLink(id) {
  ...
  async updatePaymentLink(paymentLink) {
    ...
    this.paymentLinks.set(paymentLink.id, { ...paymentLink, updatedAt: /* @__PURE__ */ new Date() });
  }
  async deletePaymentLink(id) {
...
var MockPaymentProvider = class {
  config;
  confirmedTxHashes = /* @__PURE__ */ new Set();
  pendingTxHashes = /* @__PURE__ */ new Set();
  failedTxHashes = /* @__PURE__ */ new Set();
  constructor(config = {}) {
    this.config = {
  ...
  createConfirmedResult(params) {
    ...
    const status = compareAmounts(actualAmount, params.amount) < 0 ? "underpaid" : "confirmed";
    return {
      status,
      actualAmount,
      fromAddress: "0x" + "a".repeat(40),
...
function createPortalTokenManager(config) {
  return new PortalTokenManager(config);
}
function isPortalToken(symbol) {
  return symbol.toUpperCase() === PORTAL_TOKEN.SYMBOL;
}
function formatPortalAmount(amount) {
  if (amount >= 1e6) {
